...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className="product-card bg-white shadow-lg rounded-lg overflow-hidden border border-gray-300 hover:shadow-xl transition-shadow duration-300 h-[400px] flex flex-col relative">
            {/* Discount Badge */}
            {product.discount && (
                <div className="absolute right-2 bg-red-500 text-white text-xs font-bold py-1 px-2 rounded-md border border-red-700">
                    {product.discount}% OFF
                </div>
            )}

            {/* Product Thumbnail */}
            <Link to={`/products/${product._id}`} className="relative block h-[60%] p-2">
                {thumbnail ? (
                    <img
                        src={thumbnail}
